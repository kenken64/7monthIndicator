...
class RLEnhancedBinanceFuturesBot:
    ...
    def __init__(self, symbol: str = 'SUIUSDC', leverage: int = 50, position_percentage: float = 2.0):
        ...
        # Multi-Timeframe Configuration - NEW ENHANCEMENT
        self.enable_confluence = True  # Enable multi-timeframe confluence
        self.timeframes = {
        ...
        if self.enable_confluence:
            logger.info(f"🔄 Multi-Timeframe Confluence: ENABLED")
...
⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} UTC"""
    ...
    # ========== MULTI-TIMEFRAME CONFLUENCE METHODS (NEW ENHANCEMENT) ==========
    
    def get_multi_timeframe_data(self) -> Dict[str, pd.DataFrame]:
        """Fetch data for all timeframes for confluence analysis
        
        Returns:
            Dict: Dictionary mapping timeframe names to DataFrames
        """
        if not self.enable_confluence:
            return {}
    ...
    def generate_timeframe_signal(self, df: pd.DataFrame, indicators: Dict) -> Dict:
        ...
        Args:
            ...
            indicators: Calculated indicators for this timeframe
            
        Returns:
            Dict: Signal data including direction, strength, and reasons
        """
    ...
    def analyze_confluence(self, timeframe_signals: Dict) -> Dict:
        ...
        Returns:
            Dict: Enhanced signal with confluence weighting
        """
        try:
            aligned_signals = []
            total_weight = 0
    ...
    def _combine_signals_without_rl(self, original_data: Dict, confluence_data: Dict) -> Dict:
        """Combine original and confluence signals when RL is not available
        
        Args:
            original_data: Traditional technical analysis signals
            confluence_data: Multi-timeframe confluence analysis
            
        Returns:
            Dict: Combined signal data
        """
        try:
            # If confluence is disabled or failed, use original only
            if not self.enable_confluence or confluence_data['signal'] == 0:
                return {
                    **original_data,
            ...
            # Strategy weights (without RL)
            orig_weight = 0.4
            conf_weight = 0.6
            
            # Calculate weighted signals
            orig_weighted = original_data['signal'] * original_data['strength'] * orig_weight
            conf_weighted = confluence_data['signal'] * confluence_data['strength'] * conf_weight
            ...
            # Agreement bonus
            if original_data['signal'] == confluence_data['signal'] and original_data['signal'] != 0:
                final_strength = min(6, final_strength + 1)
            
            combined_reasons = []
            if final_signal != 0:
                combined_reasons.append(f"Combined: Original({original_data['signal']}) + Confluence({confluence_data['signal']})")
            
            combined_reasons.extend(confluence_data.get('reasons', [])[:2])
            combined_reasons.extend(original_data.get('reasons', [])[:2])
    ...
    def generate_signals(self, df: pd.DataFrame, indicators: Dict) -> Dict:
        ...
        # ========== NEW: MULTI-TIMEFRAME CONFLUENCE ANALYSIS ==========
        confluence_data = {'signal': 0, 'strength': 0, 'reasons': ['Confluence disabled']}
        
        if self.enable_confluence:
            try:
                logger.info("🔍 Starting multi-timeframe confluence analysis...")
                ...
                if multi_data:
                    ...
                    for tf_name, tf_df in multi_data.items():
                        logger.debug(f"📊 Analyzing {tf_name} timeframe ({self.timeframes[tf_name]})...")
                        
                        # Calculate indicators for this timeframe
                        tf_indicators = self.calculate_indicators(tf_df)
                        if tf_indicators:
                            # Generate signal for this timeframe
                            tf_signal = self.generate_timeframe_signal(tf_df, tf_indicators)
                            timeframe_signals[tf_name] = tf_signal
                            
                            logger.debug(f"  {tf_name}: Signal={tf_signal['signal']}, Strength={tf_signal['strength']}")
                    
                    # Analyze confluence
                    if timeframe_signals:
                        confluence_data = self.analyze_confluence(timeframe_signals)
                        
                        logger.info("🎯 Multi-Timeframe Confluence Results:")
                        logger.info(f"  Confluence Signal: {confluence_data['signal']}")
                        logger.info(f"  Confluence Strength: {confluence_data['strength']}")
                        logger.info(f"  Bullish TFs: {confluence_data.get('bullish_count', 0)}")
                        logger.info(f"  Bearish TFs: {confluence_data.get('bearish_count', 0)}")
                        
                        if confluence_data['reasons']:
                            logger.info(f"  Top Reason: {confluence_data['reasons'][0]}")
                    else:
                        logger.warning("⚠️ No timeframe signals generated")
                else:
                    logger.warning("⚠️ Could not fetch multi-timeframe data")
                    
            except Exception as e:
                logger.error(f"❌ Multi-timeframe confluence analysis failed: {e}")
        
        # ========== END CONFLUENCE ANALYSIS ==========
        ...
        if RL_ENHANCEMENT_ENABLED:
            try:
                # Prepare indicators for RL (now includes confluence data)
                indicator_dict = {
                    ...
                    'ema_9': indicators['ema_9'].iloc[-1],
                    'ema_21': indicators['ema_21'].iloc[-1],
                    # NEW: Add confluence data for RL analysis
                    'confluence_signal': confluence_data['signal'],
                    'confluence_strength': confluence_data['strength'],
                    'confluence_score': confluence_data.get('confluence_score', 0)
                }
                
                # Get RL enhancement - Pass indicators AND confluence to RL system
                enhanced = rl_generator(original_signal_data, indicator_dict)
                
                # Log the enhancement with confluence info
                logger.info(f"💹 Current SUIUSDC Price: ${df['close'].iloc[-1]:.4f}")
                logger.info(f"🤖 RL Enhancement (with Multi-Timeframe):")
                logger.info(f"   Original: Signal={original_signal_data['signal']}, Strength={original_signal_data['strength']}")
                logger.info(f"   Confluence: Signal={confluence_data['signal']}, Strength={confluence_data['strength']}")
                logger.info(f"   RL Final: Signal={enhanced['signal']}, Strength={enhanced['strength']}")
                logger.info(f"   RL Reason: {enhanced['reason']}")
                
                # Store risk level for position sizing - RL determines risk level
                self._current_risk_level = enhanced.get('risk_level', 'LOW')
                
                # Enhanced signal data with confluence information
                signal_data = {
                    'signal': enhanced['signal'],
                    'strength': enhanced['strength'],
                    'reasons': [enhanced['reason']] + confluence_data.get('reasons', [])[:2] + original_signal_data.get('reasons', [])[:2],
                    'indicators': self._extract_current_indicators(indicators),
                    'rl_enhanced': True,
                    'confluence_data': confluence_data,  # NEW: Store confluence analysis
                    'original_data': original_signal_data  # NEW: Store original analysis
                }
                
            except Exception as e:
                logger.error(f"❌ RL Enhancement failed: {e}")
                # Fallback: Use confluence + original if available
                signal_data = self._combine_signals_without_rl(original_signal_data, confluence_data)
        else:
            logger.warning("⚠️ Running without RL enhancement - using traditional + confluence signals")
            # Combine original and confluence signals without RL
            signal_data = self._combine_signals_without_rl(original_signal_data, confluence_data)
