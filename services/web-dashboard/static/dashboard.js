...
async function loadDashboardData() {
    await Promise.all([
        ...
        loadMultiTimeframeConfluence(),  // NEW: Multi-timeframe confluence
        fetchNews()
    ]);
}
    ...
    try {
        ...
        if (data.success && data.data.length > 0) {
            ...
            if (data.sentiment && page === 1) {
                ...
                // Hide sentiment if not available on page 1 or if not on page 1
...
function formatDateTime(dateString) {
    ...
    try {
        // NewsAPI provides ISO 8601 formatted dates, parse directly
...
// ========== MULTI-TIMEFRAME CONFLUENCE FUNCTIONS ==========

async function loadMultiTimeframeConfluence() {
    try {
        const response = await fetch(`/api/multi-timeframe/${currentSymbol}`);
        const data = await response.json();
        
        if (data.success) {
            displayMultiTimeframeConfluence(data);
        } else {
            showMultiTimeframeError('Failed to load confluence data');
        }
    } catch (error) {
        console.error('Error loading multi-timeframe confluence:', error);
        showMultiTimeframeError('Error loading confluence data');
    }
}

function displayMultiTimeframeConfluence(data) {
    const confluence = data.confluence;
    const metrics = data.metrics;
    
    // Update main confluence status
    updateConfluenceStatus(confluence);
    
    // Update individual timeframes
    updateTimeframeData('fast', confluence.timeframes.fast);
    updateTimeframeData('medium', confluence.timeframes.medium);
    updateTimeframeData('slow', confluence.timeframes.slow);
    
    // Update alignment summary
    updateAlignmentSummary(confluence.alignment);
    
    // Update timestamp
    const timestamp = formatTimestamp(confluence.timestamp);
    document.getElementById('confluenceTimestamp').textContent = timestamp;
    
    // Hide loading message
    document.getElementById('confluenceContainer').innerHTML = '';
}

function updateConfluenceStatus(confluence) {
    // Update main signal display
    const signalText = getSignalText(confluence.signal);
    const signalColor = getSignalColor(confluence.signal);
    
    document.getElementById('confluenceSignal').textContent = signalText;
    document.getElementById('confluenceSignal').className = `font-bold text-lg ${signalColor}`;
    
    // Update icon
    const icon = getConfluenceIcon(confluence.signal);
    document.getElementById('confluenceIcon').textContent = icon;
    
    // Update reason
    document.getElementById('confluenceReason').textContent = confluence.top_reason;
    
    // Update metrics
    document.getElementById('confluenceStrength').textContent = `${confluence.strength}/6`;
    document.getElementById('confluenceScore').textContent = `${confluence.score}/3`;
    document.getElementById('confluenceConfidence').textContent = getConfidenceLevel(confluence.strength);
    
    // Update status container styling
    const statusContainer = document.getElementById('confluenceStatus');
    statusContainer.className = `mt-4 p-3 rounded-lg border ${getBorderColor(confluence.signal)}`;
}

function updateTimeframeData(timeframe, data) {
    const signalElement = document.getElementById(`${timeframe}-signal`);
    const strengthElement = document.getElementById(`${timeframe}-strength`);
    const statusElement = document.getElementById(`${timeframe}-status`);
    const progressElement = document.getElementById(`${timeframe}-progress`);
    const containerElement = document.getElementById(`timeframe-${timeframe}`);
    
    // Update signal and strength
    signalElement.textContent = data.signal;
    signalElement.className = `font-bold ${getSignalTextColor(data.signal)}`;
    strengthElement.textContent = `${data.strength}/5`;
    
    // Update status badge
    statusElement.textContent = data.status.toUpperCase();
    statusElement.className = `text-xs px-2 py-1 rounded ${getStatusBadgeColor(data.status)}`;
    
    // Update progress bar
    const progressWidth = (Math.abs(data.signal) * data.strength / 5) * 100;
    const progressColor = getProgressColor(data.status);
    progressElement.style.width = `${progressWidth}%`;
    progressElement.className = `${progressColor} h-1.5 rounded-full transition-all duration-300`;
    
    // Update container border
    containerElement.className = `p-3 rounded-lg border ${getTimeframeBorderColor(data.status)}`;
}

function updateAlignmentSummary(alignment) {
    document.getElementById('bullish-count').textContent = alignment.bullish || 0;
    document.getElementById('bearish-count').textContent = alignment.bearish || 0;
    document.getElementById('neutral-count').textContent = alignment.neutral || 0;
}

// Helper functions for styling
function getSignalText(signal) {
    switch (signal) {
        case 1: return 'BULLISH CONFLUENCE';
        case -1: return 'BEARISH CONFLUENCE';
        default: return 'NO CONFLUENCE';
    }
}
...
function getSignalTextColor(signal) {
    switch (signal) {
        case 1: return 'text-green-600';
        case -1: return 'text-red-600';
        default: return 'text-gray-600';
...
function getConfluenceIcon(signal) {
    switch (signal) {
        case 1: return '🚀';
        case -1: return '📉';
        default: return '🔄';
...
function getConfidenceLevel(strength) {
...
function getTimeframeBorderColor(status) {
    switch (status) {
        case 'bullish': return 'border-green-200';
        case 'bearish': return 'border-red-200';
        default: return 'border-gray-200';
    }
}

function showMultiTimeframeError(message) {
    const container = document.getElementById('confluenceContainer');
    container.innerHTML = `<div class="text-center text-red-500 text-sm">${message}</div>`;
    
    // Reset displays to defaults
    document.getElementById('confluenceSignal').textContent = 'ERROR';
    document.getElementById('confluenceReason').textContent = message;
    document.getElementById('confluenceStrength').textContent = '-';
    document.getElementById('confluenceScore').textContent = '-';
    document.getElementById('confluenceConfidence').textContent = '-';
    document.getElementById('confluenceTimestamp').textContent = '-';
}