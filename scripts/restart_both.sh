#!/bin/bash
# Restart Web Dashboard, RL Bot, Chart Analysis Bot, and MCP Server
...
print_header() {
    ...
    echo -e "${PURPLE}   (Web Dashboard + RL Bot + Chart Bot + MCP Server)${NC}"
    echo -e "${PURPLE}================================================${NC}"
    echo ""
}
...
if [ ! -f "start_mcp_server.sh" ]; then
    print_error "start_mcp_server.sh not found!"
    exit 1
fi

# Make sure scripts are executable
chmod +x start_web_dashboard.sh start_rl_bot.sh start_chart_bot.sh start_mcp_server.sh
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            if [ $? -eq 0 ]; then
                ...
                print_status "⚠️ MCP Server failed to start (non-critical)"
            fi
            ...
            echo -e "${BLUE}   MCP Server: tail -f mcp_server.log${NC}"
        else
